...
- 相关 `docs/features/<feature>/requirements.md`，确认目标、非目标和成功标准。
- 按需创建的根级架构和专题文档，确认长期技术结构约束；如尚未形成，则明确标为待补齐。
...
- 需求是什么、为什么做：写入 `docs/features/<feature>/requirements.md`。
- 技术结构和稳定契约：写入按需创建的根级专题文档。
- 当前执行什么、验证什么：写入当前任务事实源。
- Roadmap 只负责阶段化表达“先做什么、后做什么、怎么判断一个阶段结束”。

## Phase 完成记录

当 phase 的验收口径满足时，更新 `docs/roadmap.md` 的阶段状态和完成依据，并引用该阶段关联的 feature 归档。

工作包任务源的归档和轮转遵循 `.spec/rules/TASKS.md` 的归档契约：有项目声明的归档 workflow 时按 workflow 执行；没有时由 main agent 按 TASKS.md 的手动归档步骤执行。Roadmap phase 不单独复制任务源，也不把同一个 `final-task.md` 当作多个 phase 的归档文件。
